...
-- Shared helper: peak overlap count of active bookings for a console in a date range,
-- optionally excluding a specific booking id (used when rescheduling).
CREATE OR REPLACE FUNCTION public.booking_peak_overlap(
  _console_slug text,
  _start_date date,
  _end_date date,
...
AS $$
  SELECT COALESCE(MAX(cnt), 0)::int FROM (
    ...
    LEFT JOIN public.bookings b
      ON b.console_type = _console_slug
...
-- Refactor create_booking to use the shared helper.
CREATE OR REPLACE FUNCTION public.create_booking(
  _name text, _phone text, _console_type text, _package_type text,
  _start_date date, _end_date date, _notes text DEFAULT NULL::text
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Refactor reschedule_booking to use the shared helper.
CREATE OR REPLACE FUNCTION public.reschedule_booking(
  _booking_id uuid, _start_date date, _end_date date
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
